...
The databricks CLI bundles top level is defined by file `databricks_ci_cd/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`databricks_ci_cd/resources/model-workflow-resource.yml`)
 - batch inference workflow (`databricks_ci_cd/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`databricks_ci_cd/resources/monitoring-resource.yml`)
 - feature engineering workflow (`databricks_ci_cd/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`databricks_ci_cd/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `databricks_ci_cd/databricks.yml` and `databricks_ci_cd/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-databricks_ci_cd-model     | /dev-databricks_ci_cd-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-databricks_ci_cd-model | /staging-databricks_ci_cd-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-databricks_ci_cd-model    | /prod-databricks_ci_cd-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-databricks_ci_cd-model    | /test-databricks_ci_cd-experiment    |
...
2. Complete the `TODO` in `databricks_ci_cd/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `databricks_ci_cd/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `databricks_ci_cd/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-databricks_ci_cd-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-databricks_ci_cd-batch-inference-job`
that runs the notebook under `databricks_ci_cd/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-databricks_ci_cd-batch-inference-job`.
...
`test-databricks_ci_cd-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-databricks_ci_cd-batch-inference-job`.
